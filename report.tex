...
\begin{itemize}
    ...
    \item Regularization: Dropout (0.25) in LSTM, L2 regularization (0.001) on dense layer
    \item Training: 30 epochs with early stopping (patience = 8, monitoring validation loss)
\end{itemize}
...
The ultimate goal is to develop an \textbf{Epidemic-Guided E-STGCN} that incorporates ODE model information. However, a fundamental challenge exists: the standard EGDL approach requires fitting ODE models to \textbf{real epidemic data} and using the fitted model's outputs as features. With artificial data, there is no real data to fit the ODE model to, making the fitting process circular and not providing the intended epidemiological guidance. This highlights the need for working with real data to enable true epidemic-guided architecture.
...
\subsection{Path 1: Switch to Alternative Periodic Disease}
Transition to diseases with more readily available real-world data, such as influenza or measles. This approach offers extensive historical surveillance data (years/decades), better temporal coverage, and standardized reporting systems (CDC, WHO). With real data, we can properly fit ODE models and develop true epidemic-guided architecture that combines real observations with ODE-derived epidemiological insights. Implementation would involve adapting ODE models to flu/measles dynamics (SIR/SEIR), utilizing real surveillance data, and developing the full epidemic-guided E-STGCN framework.

\subsection{Path 2: Continue with Artificial Dengue Data}
Continue working with artificial data while addressing identified challenges through network structure experiments, enhanced irregularity mechanisms, and exploring limited epidemic-guidance workarounds.

\textbf{Preference}: Path 1 is preferred since the STGCN code is already implemented and ready, making it straightforward to adapt to a different disease with more available data.
